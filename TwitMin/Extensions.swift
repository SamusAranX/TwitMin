...
extension NSColor {
	public convenience init?(hexString: String) {
		...
		if hex.hasPrefix("#") {
            let substrIndex = hex.index(hex.startIndex, offsetBy: 1)
            hex = String(hex[substrIndex...])
		}
...
extension NSImage {
	func saveAsPngWithPath(_ path: String) -> Bool {
        ...
        imageData = imageRep!.representation(using: .png, properties: [NSBitmapImageRep.PropertyKey.interlaced: NSNumber(booleanLiteral: false)])
...
extension NSPasteboard.PasteboardType {
    static var image: NSPasteboard.PasteboardType {
        return NSPasteboard.PasteboardType(kUTTypeImage as String)
    }
}
